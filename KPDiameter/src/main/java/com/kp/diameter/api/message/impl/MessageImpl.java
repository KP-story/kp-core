...
package com.kp.diameter.api.message.impl;
...
import com.kp.diameter.api.message.impl.parser.MessageUtil;
import com.kp.diameter.api.message.impl.parser.impl.AvpSetImpl;
 ...
 */
public class MessageImpl implements Answer, Request, Loggable {
     ...
     * @param appId
     */
    public MessageImpl(int commandCode, long appId) {
    ...
    //   * @param appId
    //   */
    //  MessageImpl(LocalInfor metadata, MessageParser parser, int commandCode, long appId) {
        ...
        for (Avp a : request.getAvps()) {
            if (a.getCode() == Avp.ORIGIN_HOST) {
                try {
                    ...
                    getLogger().error("Unable to read Origin-Host AVP value for storing Routing Info", e);
                }
                ...
                try {
                    ...
                    getLogger().error("Unable to read Origin-Realm AVP value for storing Routing Info", e);
                }
            }
        }
    }
    ...
    public String getSessionId() {
        try {
            ...
            getLogger().error("Failed to fetch Session-Id", ade);
            return null;
        }
    }
    ...
    public Answer createAnswer(long resultCode) {
        ...
        try {
            ...
            getLogger().debug("Can not create answer message", e);
        }
    ...
    public ApplicationId getSingleApplicationId(long applicationId) {
        getLogger().debug("In getSingleApplicationId for application id [{}]", applicationId);
        List<ApplicationId> appIds = getApplicationIdAvps();
        getLogger().debug("Application Ids in this message are:");
        ...
        if (firstWithNonZeroVendor != null) {
            ...
            getLogger().debug("Returning [{}] as the first application id because its the first vendor specific one found", toReturn);
        } else if (firstWithZeroVendor != null) {
            toReturn = firstWithZeroVendor;
            getLogger().debug("Returning [{}] as the first application id because there are no vendor specific ones found", toReturn);
        } else {
            toReturn = firstOverall;
            getLogger().debug("Returning [{}] as the first application id because none with the requested app ids were found", toReturn);
        }

        if (toReturn == null) {
            // TODO: ammendonca: improve this (find vendor? use com.kp.common.common app list map?)
            getLogger().debug("There are no Application-Id AVPs. Using the value in the header and assuming as Auth Application-Id [{}]", this.applicationId);
    ...
    public Object clone() {
        try {
            return parser.createMessage(parser.encode(this));
        } catch (Exception e) {
    ...
    @Override
    public String getId() {
        return getSessionId() + ":" + getHopByHopIdentifier() + ":" + getEndToEndIdentifier();
    }

    @Override
    public IMessage copy(IMessage e) throws Exception {
        ...
        message.copyHeader(this);
        byte[] avpCypher = getAvps() != null ? AvpSetImpl.parser.encodeAvpSet(getAvps()) : null;
        if (avpCypher != null) {
            message.avpSet = AvpSetImpl.parser.decodeAvpSet(avpCypher);
        }
        message.commandCode = this.commandCode;
        message.state = this.state;
        message.notMutableHopByHop = this.notMutableHopByHop;
